...
class VoiceTaskAgent:
    ...
    def handle_agent_message(self, target_user_id: str, from_user_id: str,
        ...
        system_extra = (
            ...
            f"8. **转达类消息（最重要，最容易漏）**：判断标准非常简单——\n"
            f"   **「如果我不 notify，{target_display} 事后会不会因为不知道这个消息而做错事 / 尴尬 / 落空？」**\n"
            f"   会 → **必须**调 notify_my_user，把内容真的推到 {target_display} 微信。\n"
            f"   涵盖场景（不限于以下）：\n"
            f"   · 打招呼 / 问候 / 祝福：「跟 X 说晚安」「祝 X 生日快乐」\n"
            f"   · 邀约 / 安排：「请 X 今晚来吃饭」「约 X 明天见」\n"
            f"   · **信息追加 / 改口 / 更新**（最容易漏！）：「跟 X 说改一下时间」"
            f"「告诉 X 不用带酒了」「跟 X 说改成 6 点」「跟 X 说家里已经有盐了」\n"
            f"   · 通知 / 告知事实：「告诉 X 明天开会」「跟 X 说方案定了」\n"
            f"   · 委托任务：「让 X 帮忙买点东西」\n"
            f"   ⚠️ **重点：光给对方 Agent 回一句「已经转告了 / 好的收到了 / 我告诉他」不算数**——"
            f"必须**同时调 notify_my_user 工具**才是真转达。只回礼节话不调工具 = 撒谎 = 严重错误。\n"
            f"   text 里注明是谁托的，例如「{from_display_name} 让我告诉你：不用带盐了，今晚 5 点见」。\n"
            f"9. **纯问询类**（「帮我问你用户几点方便 / 有没有空 / XX 是什么」）——"
            f"如果你**不知道答案**：调 notify_my_user 把问题推给 {target_display}，然后回对方"
            f"「等我问下 {target_display} 再告诉你」（走上面规则 4）；"
            f"如果你**明确知道且不需要用户拍板**（比如「XX 是什么」的常识、公开信息）——"
            f"直接 return reply 就够，**不用**调 notify_my_user 打扰用户。"
        )
