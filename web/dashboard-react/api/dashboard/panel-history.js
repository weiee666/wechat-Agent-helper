// Vercel serverless: 看板通用历史代理
// GET /api/dashboard/panel-history?uid=&token=&conv_id=&limit=
export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(
      `${base}/panel/history?user_id=${encodeURIComponent(uid)}` +
      `&token=${encodeURIComponent(token)}` +
      `&conv_id=${encodeURIComponent(conv_id)}` +
